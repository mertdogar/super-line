...
### Packaging
Resolved 2026-06-23. The **Store interface types** (`ServerStore` / `ClientStore` / `Resource` / `Change` / `AccessRules` / clustering mode) live in **`@super-line/core`** (beside `RawConn` / `ServerTransport` / `ClientTransport` / `Adapter`). The default LWW **in-memory store** ships as its own package **`@super-line/store-memory`** (`memoryStoreServer()` / `memoryStoreClient()`) — following the transport precedent (a Store is a [[Store pair (server half / client half)]], like a transport), not the in-server adapter-default precedent. **`useResource`** lives in **`@super-line/react`**. The **CRDT binding ships from the [[super-store]] repo** as `@super-store/super-line` (the `CrdtStore` pair), importing `@super-line/core` for the interface — so **Yjs/CRDT code and deps stay out of the super-line repo entirely**, and the engine + binding iterate together on super-store's release cadence.
