...
- Status: **Superseded by [ADR-0007](0007-crdt-docs-are-typed-collections.md)** (2026-07-06) — narrowed first to CRDT doc stores by [ADR-0006](0006-collections-are-on-contract-typed-rows.md) (2026-07-05)
- Date: 2026-06-23

> **Superseded.** ADR-0006 first narrowed this decision to CRDT doc stores (LWW data became on-contract collections), and ADR-0007 then overturned its central claim — CRDT deltas proved validatable after all (the server engine holds the live doc, and the post-merge plaintext snapshot validates against a schema) — folding CRDT documents into typed collections and retiring the store family. See ADR-0007, and ADR-0008 for what that validation can honestly guarantee.
...
- **Hard gates must still route through typed requests.** Anything needing a real, preventive check (money, permissions, invariants) goes through a normal contract request — the same guidance `docs/guide/synced-state.md` (now `docs/collections/crdt-documents.md`) already gives for CRDT state, and consistent with [[Access control (accessRules)]] being enforced server-side in request-handler code.
