...
Now `room.broadcast`, `srv.publish` / `forRole(r).publish`, and the [cluster event bus](./cluster-event-bus) reach clients (and peers) on **any** node.
...
- One central service to run and coordinate — when you'd rather go broker-less, see [libp2p](./adapter-libp2p) or [ZeroMQ](./adapter-zeromq).
...
Next: [libp2p](./adapter-libp2p) · back to [Choose your backbone](./scaling-adapters).
