...
- Fans out rooms, topics, and the [cluster event bus](./cluster-event-bus) the same way, with a **gossip-replicated** directory backing `srv.cluster.*` / `srv.isOnline` (eventually-consistent, like libp2p — no central store).
...
Back to [Choose your backbone](./scaling-adapters).
