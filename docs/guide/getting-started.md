...
See [The contract](./the-contract) for the full model — directions, roles, and every interaction flavor.
...
- [Events & rooms](./events-rooms) and [Topics](./topics) — the push patterns you just used.
- [Roles & auth](./roles-auth) — give `agent` (or admin) connections a different surface.
- [React](./react) — the same contract through typed hooks.
- [Scaling & adapters](./scaling-adapters) — go multi-node with one extra line.
