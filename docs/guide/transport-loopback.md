...
Loopback supports the full lifecycle — `close()`, `terminate()`, and `server.stop()` — so reconnect and disconnect tests work too. See [Testing](./testing) for the broader test-harness patterns.

Back to [Choose your wire](./transports).
