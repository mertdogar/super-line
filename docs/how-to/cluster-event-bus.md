...
Use `server.publish` / `server.subscribe` to fan a message out across the cluster — to server-side listeners on every node *and* subscribed clients — from a single shared-topic declaration. A **shared topic** doubles as a symmetric, cluster-wide pub/sub bus with no extra API: one declaration types all three of its subscribers at once — server-side listeners, client-side subscribers, and the publish itself.
...
- **other nodes' `server.subscribe` listeners** — fire via the [adapter](/concepts/transports-and-adapters) (inbound-validated);
...
Next: [Roles & auth](/how-to/roles-auth).
