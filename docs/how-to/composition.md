...
::: tip Durable, typed state is a collection
`mergeSurfaces` merges only the **wire surface** — requests, events, and topics. A library's persisted state is a [collection](/collections/) declared on the contract, contributed with a contract-fragment plugin (`defineContractPlugin`) and locked down with server-side `policies`. See [Build a plugin](/how-to/building-plugins) for the recipe. Prefix collection names (`lib.…`) the same way you prefix keys.
:::
...
- [ ] Keys prefixed in source: requests/events/topics (`lib.join`), collection names (`lib.threads`), room names (`lib:…`)
- [ ] Surface exported via `defineSurface`; handlers exported as a factory
...
- [Build a plugin](/how-to/building-plugins) — bundle the weave (handlers · middleware · collections) into one mountable unit.
- [Plugins](/concepts/plugins) — the model behind the paired runtime bundle.
- [Test your library](/how-to/testing) — boot a real loopback server and drive it with a real client.
