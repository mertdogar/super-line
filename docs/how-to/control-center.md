...
This is also where you watch a queue. [`plugin-queue`](/how-to/plugin-queue)'s three collections — `queueJobs`, `queueSchedules`, `queueSlots` — are deny-all for clients, but the inspector is a trusted observer, so the row browser is a live job list: filter `queueJobs` by status or queue name to see what is pending, running, or failed, sort by **updated** to find the job that just moved, and read `queueSlots` to see which node holds which lease. The [`queue-cluster` example](https://github.com/mertdogar/super-line/tree/main/examples/queue-cluster) ships a Control Center alongside its dashboard for exactly this.
