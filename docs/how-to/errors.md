...
super-line carries errors end-to-end as a typed `SuperLineError`. Throw one from a handler and the client's promise rejects with the **same code** — no result-checking, no leaked internals.
...
You can also throw **custom string codes** — autocomplete keeps the built-in set while allowing your own:
...
- **Unexpected throws** — any non-`SuperLineError` thrown becomes `INTERNAL`, so server internals (stack traces, messages) are never leaked. Use [`onError`](/how-to/middleware-lifecycle#lifecycle-hooks) to log the real error server-side.
...
- [Middleware & lifecycle](/how-to/middleware-lifecycle) — where to log the real error behind an `INTERNAL`.
- [Reconnection & delivery](/concepts/reconnection-delivery) — why in-flight requests reject with `DISCONNECTED`, and what survives a drop.
- [API reference](/reference/) — the `SuperLineError` signature and `ErrorCode` union.
