...
Reach for `srv.local` when you're acting on the current process; reach for `srv.cluster` when you need the whole fleet.
...
## Ask a client a question — server → client requests
...
- [Choose an adapter](/how-to/choose-an-adapter) — make `srv.cluster.*` span more than one node.
- [Broadcast across nodes with the cluster event bus](/how-to/cluster-event-bus) — the fan-out counterpart to targeted send.
- [Inspect traffic with the Control Center](/how-to/control-center) — presence and topology in a live UI.
