...
Hook the connection lifecycle and gate operations before they reach a handler. This page covers the middleware chain (`use`) and the server lifecycle hooks (`onConnection`/`onDisconnect`/`onError`).

## Gate operations with middleware
...
These hooks are also the recommended seams for [testing](/how-to/testing) — capture the server-side `conn`, assert connects/disconnects, observe thrown errors.

::: tip One hook, many concerns
`onConnection`/`onDisconnect`/`onError` and `use` are singular here — but a [**plugin**](/concepts/plugins) multiplexes them, so any number of independent concerns (metrics, audit, an embedded library) can each register their own without hand-composition. Plugins also add a node-local **tap** (`onEvent`) that observes every request, event, and collection write with live payloads.
:::

Next: [Error handling](/how-to/errors).
