...
`@super-line/transport-websocket` provides the WebSocket transport. Other transports (HTTP/SSE, libp2p) all hand `authenticate` the same `Handshake` — see [Choose a transport](/how-to/choose-a-transport).
...
The [chat example](https://github.com/mertdogar/super-line/tree/main/examples/chat) shows a human and an AI agent sharing one room. For patterns building agent-facing surfaces, see [AI agents](/how-to/ai-agents).
