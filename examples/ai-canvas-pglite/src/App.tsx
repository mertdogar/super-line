...
// Connect back to a cluster node: ?node=2 → node-2 (:8802), else node-1 (:8801). Open two windows on two
// different nodes to watch edits CRDT-converge across nodes through Electric. Uses location.hostname so it
// also works over Tailscale / LAN, not just localhost.
const NODE_LABEL = new URL(location.href).searchParams.get('node')?.trim() === '2' ? 'node-2' : 'node-1'
const WS_URL = `ws://${location.hostname}:${NODE_LABEL === 'node-2' ? 8802 : 8801}`
...
function Board({ me }: { me: string }) {
  ...
  return (
    <div className="wrap">
      <header>
        <strong>ai-canvas-pglite</strong>
        <span>
          you are <b>{me}</b> on <b>{NODE_LABEL}</b> · {shapes.length} shapes
        </span>
      ...
      <footer>
        Open one window here (<b>node-1</b>) and another at <code>?node=2</code> (<b>node-2</b>). Drag shapes,
        double-click to delete, ask the agent — every edit CRDT-merges across both nodes through Electric, live.
      </footer>
    </div>
  )
}
