...
// only by NODE_NAME. The shared Redis adapter makes room.broadcast and presence cross nodes,
// so a message typed in a tab on node-1 reaches a tab on node-2.
...
server.listen(PORT, () => console.log(`[${NODE}] chat node up on :${PORT} (redis ${REDIS_URL})`))
