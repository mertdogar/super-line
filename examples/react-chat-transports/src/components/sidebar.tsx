...
export function Sidebar({ channels, joined, activeId, onSelect, onSignOut }: SidebarProps): React.JSX.Element {
  const me = useMe()
  const users = useUsers()
  // Live presence with no app code: plugin-auth keeps these rows from real connection sessions, so a
  // tab on libp2p and a tab on HTTP show up in each other's list exactly the same.
  const online = useOnline()
  const joinedSet = useMemo(() => new Set(joined), [joined])
  const onlineUsers = useMemo(
    () =>
      [...online]
        .map((id) => ({ id, name: users.get(id)?.displayName ?? 'unknown' }))
    ...
    <aside className="flex h-full w-64 shrink-0 flex-col bg-sidebar text-sidebar-foreground">
      ...
      <div className="flex-1 overflow-y-auto py-3">
        ...
        <nav className="space-y-0.5 px-2">
          {channels.map((c) => (
            <ChannelRow key={c.id} channel={c} joined={joinedSet.has(c.id)} active={c.id === activeId} onSelect={onSelect} />
          ))}
        </nav>
      </div>
        ...
        <ul className="space-y-1.5">
          {onlineUsers.map((u) => (
            <li key={u.id} className="flex items-center gap-2 text-sm">
              ...
              <span className={cn('truncate', u.id === me ? 'font-semibold' : 'text-sidebar-muted')}>
                {u.name}
                {u.id === me && ' (you)'}
              </span>
            </li>
