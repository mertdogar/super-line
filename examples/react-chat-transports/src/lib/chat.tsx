...
import type { chat, Membership, User, UserPresence } from '@/contract'
...
export { useChannels, useMembers, useMessages, useChannelResources } from '@super-line/plugin-chat/react'
 ...
 * directory, live presence, my memberships) over the registered `useCollection`.
 */
export function ChatProvider({ me, children }: { me: string; children: ReactNode }): ReactNode {
  ...
  const { rows: presence } = useCollection('userPresence')
  const online = useMemo(() => {
    const cutoff = Date.now() - USER_PRESENCE_LIVE_MS
    return new Set(
      (presence as UserPresence[]).filter((p) => p.connectedAt !== null && (p.lastSeenAt ?? 0) > cutoff).map((p) => p.userId),
    )
  }, [presence])
