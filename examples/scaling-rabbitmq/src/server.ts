...
// differing only by env. The shared RabbitMQ adapter fans rooms, topics, and the
// cluster event bus out across all three — the broker does selective routing.
...
server.listen(PORT, () => console.log(`[${NODE}] up on :${PORT} (rabbitmq ${RABBITMQ_URL})`))
