...
| Connection env | `env:` schema in a role → server-vended, CLIENT-VISIBLE `conn.env`; seeded by `authenticate`'s `env`, updated live via `conn.setEnv(v)` / `srv.toConn(id).setEnv(v)` / `srv.toUser(uid).setEnv(v)`; client reads `client.env.current`/`.ready`/`.subscribe`, React `useEnv()`. Clients cannot write it |
...
| Control Center | `createSuperLineServer(api, { …, plugins: [inspector()] })` (from `@super-line/plugin-inspector`; `inspector({ redact: ['token'] })` to mask fields; **`SUPER_LINE_INSPECTOR_PASSWORD` or `inspector({ auth })` to lock it — unlocked otherwise**), then `npx @super-line/control-center` → cluster-wide live feed of `msg.*` + collection/CRDT traffic + topology |
| Client devtools | `plugins: [devtoolsPlugin()]` on the CLIENT (from `@super-line/plugin-devtools`) + the super-line DevTools panel → this tab's frames, in-flight/queued requests, reconnect backoff, per-subscription routing, live rows, CRDT contents. **No server config**; complements the Control Center rather than duplicating it. Any client plugin can tap the same stream with `onClientSideEvent` (note: NOT `onEvent`, which is the server's) |
| React | Declare `Register` once on `'@super-line/react'` → module-level `SuperLineProvider` / `useSuperLineClient` (StrictMode-safe ownership) / `useRequest(method, input?)` (input ⇒ auto-fetch; `{ data, error, loading, refetch, call }`) / `useEvent` / `useSubscription` / `useCollection` (rows + `ready`/`batch`/`handle`/`sub`; `query: null` = idle) / `useDoc` (CRDT doc; id may be null or a resolver; `ready`/`error`/`handle`) / `useLiveQuery` / `useEnv`. Factory escape hatch: `createSuperLineHooks<typeof api, 'user'>()` |
...
| Collaborative rich text | A described string is **replaced** whole per write, so it cannot hold text two people type at once. Use a **native root**: `crdt: { mode: 'document', validate: false }`, then `Collaboration.configure({ document: yDocOf(handle), field: 'body' })` — Tiptap's `field` IS the Yjs root key, and there is **no provider** (super-line syncs it). Invisible to validation/queries/inspector; keep validatable state out of that doc (a reject swaps the `Y.Doc` and orphans the editor) and put the title in a row collection beside it |
| Auth (`plugin-auth`) | Contract `plugins: [authContract()]` (adds `guest` + 7 identity collections; `users`/`userPresence` public, rest deny-all). Server `const authKit = auth({ contract, collections, jwt?, rejectUnauthenticated?, hooks? })` → **`nodeKey: 'app-replica-1'`** (REQUIRED, stable across restarts — the plugin throws at boot without it) + `authenticate: authKit.authenticate` + `identify: authKit.identify` + `plugins: [authKit.plugin]` (SAME collection backend). Client `authClient({ authedRole, connect, tokenParam?, resolveToken? })` → `signIn`/`signUp`/`signOut`/`reauthenticate()` + `state` (`status`·`pending`·`error`); React: `declare module '@super-line/react' { interface Register { contract; role } }` once, then `<SuperLineAuthProvider>` (from `@super-line/plugin-auth/react`, with `useAuth`) owns the session AND feeds the shared binding — data hooks (`useMaybeClient`/`useCollection`/`useDoc`/…) import from `@super-line/react`, no client threaded anywhere, hooks idle before auth. Sessions + `userPresence` · data-driven `roles[]` · API keys (`slp_`) · server-minted signed/sealed assertions (`authKit.tokens.mintSigned`/`mintSealed`/`verify`) · server-side `hooks` · `authKit.revoke(uid)` |
| Chat (`plugin-chat`) | Channels · membership · messages, **requires `plugin-auth`**. Contract `plugins: [authContract(), chatContract()]` (6 collections; body via `chatContract({ content })`). Server `const chatKit = chat({ contract, hooks?, resources? })` + `plugins: [authKit.plugin, chatKit.plugin]` — every mutation is a server-authoritative hookable request, collections are client-read-only. Kit: `chatKit.channels/members/messages/resources` (+ access resolvers `resources.canAccessDoc`/`docIdsOf`). Client `chatClient(client, { userId })`; React `createChatHooks()` → 11 null-tolerant hooks (`useChannels`/`useMessages`/`useMe`/`useChannelBusy`/`useChatHistory`/…). **Streamed turns**: `chat.stream(channelId)` writer → viewers get one assembled feed (`parts` + `status`); `pipeUIMessageStream` bridges AI SDK v6. **Bots are 3 calls**: `provisionChatBot(authKit, chatKit, {name})` · `onChatMessage(chat, handler)` · `/mastra`'s `mastraEngine({agent, subagents})`. **Channel resources**: register a CRDT collection as a kind → membership-gated docs a human (`useDoc`) and an agent (`writeResource`) co-edit |
| Debug logging | `enableSuperLineLogging({ level: 'debug' })` from `@super-line/core` — pretty, secret-redacting console over categories `['super-line','<pkg>','<subsystem>']`. Owns the process-global LogTape config: use it **or** your own `configure()` (then add a `[LOG_ROOT]` logger wrapped in `redactByField(sink, SUPER_LINE_REDACT_FIELDS)`), never both |
...
- **PREFER** `@super-line/plugin-auth` for real login (email/password · sessions · roles · API keys · bearer assertions): wire `plugins: [authContract()]` on the contract, a stable **`nodeKey`** + `authenticate`/`identify`/`plugins: [authKit.plugin]` on the server (SAME collection backend passed to `auth()`), `authClient` / `<SuperLineAuthProvider>` on the client. Role AND credential are frozen at connect, so every identity change — login, logout, **account switch** (`reauthenticate()`) — is a **reconnect**, not an upgrade.
...
- **A schema cannot describe collaborative text — don't try.** Fields merge per field; text must merge per character, which is a **native root** (`yDocOf(handle)`, bound beside the described root). Nothing validatable is then left in the described root, so declare `crdt: { validate: false }` — validating per keystroke is unaffordable regardless, the check being proportional to document size and history. **The policy then gates who may write, and nothing gates what.** A Yjs root's kind is fixed on first use, so keep the root name in one shared constant.
- **plugin-auth throws at boot without a stable `nodeKey`** (it keys per-node session reconciliation on it). Use a per-replica name that survives restarts, never a random id.
- **A bad credential DEGRADES TO GUEST by default — the connection is *accepted*.** An expired/forged token resolves to `guest`, so a client built for `user` sees no connect error and just `NOT_FOUND`s every call. Confirm with `whoami()` (what `authClient` does) or set `rejectUnauthenticated: true` to throw `UNAUTHORIZED` at connect. A credential-*less* connect still resolves guest either way; on the client a rejection surfaces as `state.error`.
- **`env` is server-vended, client-visible, and read-only to the client.** Only `conn.setEnv`/`srv.toConn(id).setEnv`/`srv.toUser(uid).setEnv` push a value (full-value replace); there's no client-side `set`. The Control Center masks `env` values by default (`•••`) — the opposite of `ctx`/`data`'s deny-list `redact` — allow-list safe keys via `inspector({ revealEnvKeys: [...] })`.
