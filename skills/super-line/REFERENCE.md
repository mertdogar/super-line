...
`queue({ queues, pollIntervalMs?, shutdownGraceMs?, maxCatchUp?, timezone? })` returns `{ contract, plugin, queue, enqueue, get, list, cancel, retry, schedules }`. Each queue declares input/result schemas, positive `concurrency`, and an OPTIONAL `worker`; workers receive `{ jobId, attempt, signal }`. `queue(name)` is one queue's namespace — `name`, `hasWorker`, `setWorker(fn)`, `enqueue(input, options?)`, `list(query?)` and `schedules.create`/`.list`, all scoped to that queue (`get`/`cancel`/`retry` stay on the kit, being keyed by job id). `setWorker` binds late (for a contract package that cannot import the server's worker modules), applies immediately, is per node, and wins over an inline `worker`. A node never claims a queue it has not bound: those jobs stay `queued` for a node that has one rather than failing, and an unbound node still reaps expired leases and still advances schedules. The contract adds deny-all `queueJobs`, `queueSchedules`, and `queueSlots`. Schedules expose `create`, `update`, `pause`, `resume`, `delete`, `trigger`, `get`, and `list`. Jobs are at least once; `runId` fences leased runs. Use `pgliteCollections` with shared Postgres for cluster-wide claims and cron.
