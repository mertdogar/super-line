...
| Store (server) | `stores: { docs: <server-half> }`, then `srv.store('docs').create/grant/revoke/write/read/list/delete` (server-authoritative, off-contract). **Halves — model · durability · clustering:** `memoryStoreServer()` LWW·mem·relay · `syncStoreServer()` CRDT·mem·relay · `sqliteStoreServer({ file })` LWW·durable·relay · `await pgliteStoreServer({ pgUrl, electricUrl? })` LWW·**self** · `await syncPgliteStoreServer({ pgUrl, electricUrl? })` CRDT·**self** (pglite factories are **async** — `await`). Durable CRDT·relay is no longer a store — use a CRDT document collection (↓) |
...
| Store (client) | client half matches the server's **model**: `memoryStoreClient({ origin? })` for LWW (memory/sqlite/pglite) · `syncStoreClient({ resolveOptions?, origin? })` for CRDT (sync/sync-pglite). `const h = client.store('docs').open(id)` → `{ getSnapshot, subscribe, set, update, delete(path), deleted, ready, close }`; one-shot `client.store('docs').read/write(id, …)` |
...
| **CRDT document collections** (ADR-0007 — the CRDT-store successor) | Declare with a `crdt` key (no `key`): `collections: { scenes: { schema, crdt: { mode: 'document' } } }`. **Opened by id, not queried** (whole-doc merge). Server takes a SEPARATE `crdtCollections:` backend (`crdtMemoryCollections()` · `await crdtLibsqlCollections({url})`) + guard-shaped `policies` (`read(principal,id,snapshot?)→bool` / `write(principal,id)→bool`, deny-by-default). **Validate-before-commit** at ingress — deltas ARE schema-checked (overturns ADR-0003); a reject → the client resyncs. Creation is server-only (`srv.collection(n).create(id,data)`). Client needs `crdtCollections: crdtCollectionsClient()`; `client.collection('scenes').open(id)` → DocHandle; React `useDoc`. See REFERENCE.md → CRDT document collections |
