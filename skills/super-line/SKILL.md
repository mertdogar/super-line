...
| Store (server) | configure `stores: { docs: memoryStoreServer() }` (LWW · or `syncStoreServer()` CRDT · or `sqliteStoreServer({ file })` durable); then `srv.store('docs').create/grant/revoke/write/read/list/delete` (server-authoritative, off-contract) |
| Store co-writer (server) | `const h = srv.store('docs').open(id, { origin? })` → reactive `{ getSnapshot, subscribe, update, set, delete(path), close }` — in-process, the **only** way to delete a key server-side |
| Store (client) | configure `stores: { docs: memoryStoreClient() }`; `const h = client.store('docs').open(id)` → `{ getSnapshot, subscribe, set, update, delete(path), ready, close }`; one-shot `client.store('docs').read/write(id, …)` |
| Store (React) | `const { data, set, update, delete: del } = useResource<T>('docs', id)` |
