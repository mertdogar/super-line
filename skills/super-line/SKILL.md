...
| **Collections** (typed rows) | Declare **on the contract** (`collections: { messages: { schema, key, references? } }`); server takes ONE backend (`memoryCollections()` · `sqliteCollections({file, collections})` relay · `await pgliteCollections({pgUrl, collections})` **self** — SQL backends derive one typed table per collection from the contract's `collections` map) + row `policies` (deny-by-default `read`→IR filter / `write`→bool). Client `client.collection('messages').subscribe({filter,orderBy,limit})` → live row-set + `insert/update/delete/batch`; React `useCollection`. Joins/live-queries via `@super-line/tanstack-db` + TanStack DB. `srv.collection(n)` server co-write. See REFERENCE.md → Collections |
