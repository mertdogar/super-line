---
name: super-line
description: Use when building realtime features with super-line, including durable server-side queues and workers through @super-line/plugin-queue.
---
...
| Client devtools (debug) | `plugins: [devtoolsPlugin()]` on the CLIENT (from `@super-line/plugin-devtools`), then open the super-line DevTools panel → this tab's frames, in-flight requests, reconnect backoff, per-subscription routing, live collection rows and CRDT document contents. Needs **no server config** and works against a server with no inspector; shows what the server structurally cannot see |
| React | `declare module '@super-line/react' { interface Register { contract: typeof api; role: 'user' } }` ONCE, then module-level `<SuperLineProvider client={…}>` + `useRequest`/`useEvent`/`useSubscription`/`useCollection`/`useDoc`/`useEnv` + `useSuperLineClient(make)` (StrictMode-safe client ownership). Factory escape hatch for multi-contract/tests: `createSuperLineHooks<typeof api, 'user'>()` |
| **Collections** (typed rows) | Declare **on the contract** (`collections: { messages: { schema, key, references? } }`); server takes ONE backend (`memoryCollections()` · `sqliteCollections({file, collections})` relay · `await pgliteCollections({pgUrl, collections})` **self** — SQL backends derive one typed table per collection from the contract's `collections` map, reading each schema's Standard JSON Schema — Zod 4.2+ · Zod Mini 4.2+ · ArkType 2.1.28+ · VineJS 4.3+ · Sury 11+ · stnl 2.1+ · Valibot 1.2+ wrapped in `toStandardJsonSchema()`; a validator whose library lacks the companion spec still works, its collection just falls back to key + one `_sl_data` JSON column) + row `policies` (deny-by-default `read`→IR filter / `write`→bool). Client `client.collection('messages').subscribe({filter,orderBy,limit})` → live row-set + `insert/update/delete/batch`; React `useCollection`. Joins/live-queries via `@super-line/tanstack-db` + TanStack DB. `srv.collection(n)` server co-write. See REFERENCE.md → Collections |
...
| **Collaborative rich text** | A described string is **replaced** whole on every write, so it cannot hold text two people type in at once. Put the text in a **native root** — a CRDT type beside the described root: declare `crdt: { mode: 'document', validate: false }`, then `Collaboration.configure({ document: yDocOf(handle), field: 'body' })` (Tiptap's `field` IS the Yjs root key; **no provider** — super-line already syncs it). Invisible to validation, the queryable projection and the inspector. Keep validatable state OUT of that document (a reject rebuilds the `Y.Doc` and orphans the editor) — put the title in a row collection beside it. See REFERENCE.md → CRDT document collections |
| **Auth** (`@super-line/plugin-auth`) | Contract: `plugins: [authContract()]` (adds the `guest` role + **7** identity collections — `users` + `userPresence` public, the rest deny-all). Server: `const authKit = auth({ contract, collections: backend, jwt?, rejectUnauthenticated?, hooks?, sendPasswordReset? })` then `nodeKey: 'app-replica-1'` (**REQUIRED — stable across restarts, or the plugin throws at boot**) + `authenticate: authKit.authenticate` + `identify: authKit.identify` + `plugins: [authKit.plugin]` — pass the **same** collection backend to `auth()` and the server. Client: `authClient({ authedRole: 'user', connect, tokenParam?, resolveToken? })` → `signIn`/`signUp`/`signOut`/`reauthenticate` + `state` (`status`/`pending`/`error`); React: declare `Register` once, then `<SuperLineAuthProvider>` owns the session **and** feeds every data hook (`useAuth`/`useClient`/`useCollection`/`useDoc`/…) — no client threaded anywhere. Email/password · sessions + `userPresence` · data-driven `roles[]` · API keys (`slp_`) · server-minted **signed/sealed** assertions (`authKit.tokens.mintSigned`/`mintSealed`/`verify`) · server-side `hooks` · `authKit.revoke(userId)`. See REFERENCE.md → @super-line/plugin-auth |
| Debug logging | `enableSuperLineLogging({ level: 'debug' })` from `@super-line/core` — pretty, **secret-redacting** console for super-line's internals (categories `['super-line','<pkg>','<subsystem>']`). It owns the process-global LogTape config: use it **or** your own `configure()`, never both. See REFERENCE.md → Logging |
| **Chat** (`@super-line/plugin-chat`) | Chat backbone: channels (public/private), owner/member membership, messages (send/edit/delete). Contract: `plugins: [authContract(), chatContract()]` (REQUIRES plugin-auth; body host-parametrized via `chatContract({ content })`, default text; `{ data }` types custom data parts). Server: `const chatKit = chat({ contract, hooks? })` + `plugins: [authKit.plugin, chatKit.plugin]` — every mutation is a hookable, server-authoritative request; collections are client-read-only. Imperative kit `chatKit.channels/members/messages/resources` (hooks fire, `initiator: 'server'`) — incl. `members.get(channelId,userId)` and the resource **access resolvers** `resources.channelsOfDoc`/`canAccessDoc`/`docIdsOf` (shared verbatim with the plugin's own kind guards, so a host RLS filter can't drift from them). Client: `chatClient(client, { userId })` (no TanStack/React dep, owns membership re-subscribe); React `createChatHooks()` → 11 hooks: `useChat`/`useChannels`/`useMembers`/`useMessages`/`useMessageParts`/`useMe`/`useChannelBusy`/`useChatHistory` (keyset pagination)/`useChannelResources`/`useResourcePresence` + `ChatProvider` — every channel/message-scoped hook is **null-tolerant** (a null id = idle: `[]`, no subscription). **Streaming messages**: a message can store an ENTIRE agent turn as typed parts (text/reasoning/tool, subagent trees via `parent`) — producer `chat.stream(channelId)` writer (or `chatKit.messages.stream`), viewers get the same `messages()` feed assembled (`parts`+`status`, token-smooth deltas over ~1s row checkpoints — late-join/reload safe); `pipeUIMessageStream` bridges AI SDK v6 in one line (`mapDataPart` turns a host data/file/source chunk into a custom part). Deleting a still-`streaming` message **settles it first** — a turn never blinks out mid-flight, and a write behind the settle gets `CONFLICT`. AI agents = users; `/ai` ships `chatAgentTools(client, { management? })` → a Vercel AI SDK `ToolSet` (client-side, server re-authorizes every call, `ai` optional peer). **Bots are 3 calls**: `provisionChatBot(authKit, chatKit, {name})` (`/server`, restart-idempotent identity+key) · `onChatMessage(chat, handler, {channels?})` (`/client` loop: auto-join, dedup, model-ready history, turns serialized per channel) · `/mastra`'s `mastraEngine({agent, subagents})` streams a PLAIN Mastra supervisor+workers as one nested-parts message (delegate tool injected via toolsets — never on your Agent; `@mastra/core` optional peer; `pipeMastraStream` = single-lane sibling). Per-agent config (maxSteps · thinking · memory) stays on YOUR Agent via Mastra `defaultOptions` — a function of the forwarded `requestContext`, so per-channel memory goes on the root Agent only (workers stateless by construction; with memory on, pass only the new turn — Mastra recalls the past). **Channel resources**: host-declared CRDT collections become channel-attached shared objects — `chat({ resources: { kinds: { canvas: { collection, lifecycle: 'owned'|'linked', init } } } })`; registering a kind auto-contributes membership-gated doc policies (do NOT also policy that collection — boot-throw), enables `createResource` (create-or-attach; `linked` = host doc ids, multi-channel, never chat-deleted; `owned` dies with the channel) + the delete cascade; registry rows in `resources`, cards in `metadata.resource` (bot-loop skips them), acked `writeResource` path-ops (honest VALIDATION for agents; DocHandle stays optimistic), who's-open presence (`resourcePresence` + `announceResource` + `useResourcePresence`, reap via `chatKit.resources.sweepPresence`); /ai adds list/read/create/detach/write_resource. See REFERENCE.md → @super-line/plugin-chat |
...
| Server-vended, client-visible state | declare `env:` schema in a role block → seeded by `authenticate`'s `env`, updated live via `conn.setEnv(v)` / `srv.toConn(id).setEnv(v)` / `srv.toUser(uid).setEnv(v)`; read as `client.env.current`/`.ready`/`.subscribe(cb)`, React `useEnv()`. **Server-vended — clients cannot write it.** See REFERENCE.md → Connection env |
...
- **TWO co-writer doors for an agent — pick by whether the agent is a channel MEMBER.** (a) `srv.collection(n).open(id)` — the server co-writer above: **trusted, unvalidated, off the membership model**, best for an in-process privileged actor. (b) `@super-line/plugin-chat`'s **`writeResource`** (the `write_resource` tool) — the agent acts as an ordinary channel **member**: membership+registry gated, **best-effort validated with an honest `VALIDATION`** the model can read, over the bot's own connection so the server re-authorizes it. For an agent that lives *inside* a channel like any user (the `chat-supervisor` shape), reach for `writeResource`. **Channel resources** (`chat({ resources: { kinds } })`) are how a host-declared CRDT collection becomes channel-attached: registering a kind auto-contributes membership-gated policies (don't also policy that collection — boot-throw), the human edits via `client.collection(n).open(id)` / `useDoc`, the agent via `writeResource`. See REFERENCE.md → @super-line/plugin-chat (Channel resources); guide `/how-to/chat-resources`; Tutorial 6.
- **PREFER** `@super-line/plugin-auth` over hand-rolling sessions when you need real login: it wires as a contract fragment (`authContract()`) + a server plugin (`authKit.plugin`), keeps all identity in collections, and hides super-line's guest→user reconnect. Because a connection's **role is frozen at connect**, login is a **reconnect, not an upgrade** — `signIn`/`signUp` tear down the guest socket and reconnect as the authed role. Pass the SAME `CollectionStore` to `auth({ collections })` and to the server, and give the server a **stable `nodeKey`** — the plugin keys per-node session reconciliation on it and **throws at boot without one**.
...
- **A CRDT document's schema cannot describe collaborative text, and should not try.** Fields are merged per field; text has to merge per character, which needs a **native root** (`yDocOf(handle)` → bind an editor to a root beside the described one). Since nothing in the described root remains to check, such a collection declares `crdt: { validate: false }` — and validating per keystroke would be unaffordable anyway, the check being proportional to document size and history. That leaves the **policy as the only gate on content**: it decides who may write, and nothing then decides what.
...
- **plugin-auth throws at boot without a stable `nodeKey`.** `createSuperLineServer(app, { nodeKey: 'app-replica-1', … })` — a per-replica name that survives restarts, never a random id (a changing value leaks prior-boot sessions). This is the #1 thing forgotten when wiring auth.
- **A bad credential DEGRADES TO GUEST by default — the connection is *accepted*.** An expired/forged/deactivated token resolves to `guest`, so a client built for `user` doesn't see a connect error; it just `NOT_FOUND`s on every call, which reads like a contract bug. Either confirm with `whoami()` (shared; `null` for a guest — what `authClient` does) or set `rejectUnauthenticated: true` so a *presented*-but-invalid credential throws `UNAUTHORIZED` at connect. A credential-*less* connect still resolves guest either way. On the client, a rejected token surfaces as `state.error` — render a reconnect banner off it.
- **`env` is server-vended and client-visible — the client cannot write it.** There's no client-side `set`; only `conn.setEnv` / `srv.toConn(id).setEnv` / `srv.toUser(uid).setEnv` push a new value (full-value replace, last-write-wins). It's the visibility-mirror of `conn.data` (server-only) — use it to hand a connection working credentials, wired into outbound calls by the client's OWN code, never surfaced to an LLM. The Control Center **masks `env` values by default** (`•••` per key, shape still shown) — the opposite of `ctx`/`data`'s deny-list `redact` — allow-list safe keys with `inspector({ revealEnvKeys: [...] })`.
