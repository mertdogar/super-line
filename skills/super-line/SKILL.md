...
| Control Center (debug) | `webSocketServerTransport({ server, inspector: true })` + `createSuperLineServer(api, { …, inspector: true })` (or `{ redact: ['token'] }` to mask fields), then `npx @super-line/control-center` → cluster-wide live feed of `msg.*` traffic + topology |
